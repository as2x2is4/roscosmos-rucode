...
def run_change_det(infile):
    ...
    bound_mask = np.array(indataset.GetRasterBand(1).ReadAsArray())
    bound_mask = cv2.blur((bound_mask == 0) * 255.0,(9,9))
    num_zeros = (bound_mask == 0).sum()
    num_others = (bound_mask > 0).sum()
    ...
    for iBand in range(0, indataset.RasterCount):
        ...
        band_cloud_mask = (outband_arr >= 255*5.999)*255
    ...
    for iBand in range(0, indataset.RasterCount):
        ...
        gradmean = grad.mean()
        gradmean = gradmean*(num_zeros + num_others)/num_others
        grad = grad * 32.0 / gradmean
    ...
    gradmean = grad_colors.mean()
    gradmean = gradmean*(num_zeros + num_others)/num_others
    grad_colors = grad_colors * 32.0 / gradmean
    #grad_colors = grad_colors * 255.0 / (grad_colors.max())
    grad_colors[grad_colors > 255] = 255
    grad_colors = (grad_colors).astype('uint8')

    #grad_colors = cv2.equalizeHist(grad_colors) ### OPENCV
    ...
    #band_grad_mask = cv2.equalizeHist(band_grad_mask) ### OPENCV
    ...
    band_grad_mask_thr = (band_grad_mask >= 0.1 * 255)*255
    nb_components, output, stats, centroids = cv2.connectedComponentsWithStats(band_grad_mask_thr.astype('uint8'), connectivity=8)
    sizes = stats[1:, -1]; nb_components = nb_components - 1
    # minimum size of particles we want to keep (number of pixels)
    max_size = 2500
    min_size = 75
    #your answer image
    cc_filter = np.zeros((band_grad_mask_thr.shape))
    #for every component in the image, you keep it only if it's above min_size
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(77,77))
    for i in range(0, nb_components):
        if sizes[i] >= min_size and sizes[i] <= max_size:
            cc_morph = cv2.morphologyEx(((output == i + 1) * 255).astype('uint8'), cv2.MORPH_CLOSE, kernel)
            cc_filter[cc_morph > 0] = 255
            #cc_filter[output == i + 1] = 255

    band_grad_mask[cc_filter == 0] = 0

    outdataset = gdal.GetDriverByName('GTiff').Create( outfolder + outfile + "_XXXX_INTERSECT_GRADS_FILTER" + ".tif" , indataset.RasterXSize, indataset.RasterYSize, 1, gdal.GDT_Byte)
    outdataset.GetRasterBand(1).WriteArray(band_grad_mask)

    # kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(77,77))
    # band_grad_mask = cv2.morphologyEx(((band_grad_mask > 0) * 255).astype('uint8'), cv2.MORPH_CLOSE, kernel)

    intersect = np.minimum(band_color_dif, cc_filter)
    ...
    intersect = (intersect > 0.5 * 255) * 255
