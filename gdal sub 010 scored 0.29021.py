...
def run_change_det(infile):
    ...
    band_grad_mask = band_grad_mask * 0.25
    ...
    band_grad_mask = (band_grad_mask > 64)*255
    band_grad_mask = cv2.medianBlur(band_grad_mask.astype('uint8'), 11)
    # kernel = np.ones((25,25),np.uint8)
    # band_grad_mask = cv2.morphologyEx(band_grad_mask.astype('uint8'), cv2.MORPH_CLOSE, kernel)
    band_grad_mask = cv2.blur((band_grad_mask > 0)*100.0,(33,33))
